...
# ============================================================================
# Playwright Acceptance enforcement (extends B17) — autogenerated block.
# Source of truth: .playwright-banned-patterns at repo root (name|regex per line).
# Scans staged *.spec.ts / *.spec.js files for:
#   - missing first-line operator-outcome header
#   - banned assertion patterns (see CLAUDE.md § Playwright Acceptance)
# ============================================================================
...
if [ -f "$PW_PATTERNS_FILE" ]; then
    ...
    for spec in $PW_STAGED; do
        [ -f "$spec" ] || continue
        PW_CONTENT=$(git show :"$spec" 2>/dev/null || true)
        [ -z "$PW_CONTENT" ] && continue

        FIRST_LINE=$(printf '%s\n' "$PW_CONTENT" | head -n1)
        if ! printf '%s' "$FIRST_LINE" | grep -qE '^//[[:space:]]*Operator-visible outcome:'; then
            PW_FOUND+="  $spec:1 (missing-operator-outcome-header — first line must be '// Operator-visible outcome: <specific values>')"$'\n'
        fi
        ...
        while IFS='|' read -r pw_name pw_regex; do
            ...
            [ -z "$pw_regex" ] && continue
            PW_MATCHES=$(printf '%s\n' "$PW_CONTENT" | grep -nE "$pw_regex" || true)
            if [ -n "$PW_MATCHES" ]; then
                while IFS= read -r pw_line; do
                    pw_lineno=$(printf '%s' "$pw_line" | cut -d: -f1)
                    PW_FOUND+="  $spec:$pw_lineno ($pw_name)"$'\n'
                done <<< "$PW_MATCHES"
            fi
    ...
    if [ -n "$PW_FOUND" ]; then
        echo ""
        echo "BLOCKED: Playwright Acceptance violations (see CLAUDE.md § Playwright Acceptance)."
        echo "Each .spec.ts/.spec.js must begin with '// Operator-visible outcome: <specific values>'."
        echo "Banned assertion patterns fail acceptance even when green."
        echo "Offending file(s) and pattern(s):"
        printf '%s' "$PW_FOUND"
        exit 1
    fi
fi
