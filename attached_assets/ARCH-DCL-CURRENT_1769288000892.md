...
NLQ enables question-based definition matching with computed answers.
...
│                          ↓                                       │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  Answerability Ranking (Keyword-Based)                    │   │
│  │  - KEYWORD_MAP: definition → keywords                     │   │
│  │  - PRIORITY_PHRASES: high-confidence matches              │   │
...
- **Routes:** `backend/nlq/routes.py`
- **Keyword Maps:** `KEYWORD_MAP`, `PRIORITY_PHRASES` dictionaries

### How Scoring Works

1. **Priority Phrase Match** → Base score 85%+
   - Example: "arr" in question → `finops.arr` gets 85%+
   
2. **Keyword Matching** → Score = min(matches/3, 75%)
   - Multiple keyword matches increase score
   
3. **Best Match Wins** → Highest-scoring definition is selected
...
2. **Keyword-only matching** - No semantic/embedding-based similarity
3. **Hardcoded keywords** - Must manually add keywords for new definitions
