...
def test_combining_income_statement(client):
    ...
    resp = client.get("/api/dcl/reports/v2/combining/income-statement", params={"period": "2025-Q1"})
...
def test_combining_balance_sheet(client):
    ...
    resp = client.get("/api/dcl/reports/v2/combining/balance-sheet", params={"period": "2025-Q1"})
    assert resp.status_code == 200
    data = resp.json()
...
def test_combining_cash_flow(client):
    ...
    resp = client.get("/api/dcl/reports/v2/combining/cash-flow", params={"period": "2025-Q1"})
    assert resp.status_code == 200
    data = resp.json()
    assert "identity_check" in data
    assert data["identity_check"]["passed"] is True
...
def test_cofa_adjustments(client):
    ...
    resp = client.get("/api/dcl/reports/v2/cofa-adjustments", params={"period": "2025-Q1"})
    assert resp.status_code == 200
    data = resp.json()
...
def test_overlap_summary(client):
    ...
    resp = client.get("/api/dcl/reports/v2/overlap/summary")
    assert resp.status_code == 200
    data = resp.json()
...
def test_cross_sell_summary(client):
    ...
    resp = client.get("/api/dcl/reports/v2/cross-sell/summary")
    assert resp.status_code == 200
    data = resp.json()
...
def test_ebitda_bridge(client):
    ...
    resp = client.get("/api/dcl/reports/v2/bridge")
    assert resp.status_code == 200
    data = resp.json()
...
def test_qoe(client):
    ...
    resp = client.get("/api/dcl/reports/v2/qoe")
    assert resp.status_code == 200
    data = resp.json()
    assert "reported_ebitda" in data
    assert "adjusted_ebitda" in data
...
def test_revenue_bridge(client):
    ...
    resp = client.get("/api/dcl/reports/v2/revenue-bridge/yoy")
    assert resp.status_code == 200
    data = resp.json()
...
def test_resolution_create_workspaces(client):
    ...
    resp = client.post("/api/dcl/resolution/v2/create-workspaces", json={})
    assert resp.status_code == 200
    data = resp.json()
...
def test_resolution_list_workspaces(client):
    ...
    resp = client.get("/api/dcl/resolution/v2/workspaces")
    assert resp.status_code == 200
    data = resp.json()
