...
VENDOR_OVERLAP = 170
EMPLOYEE_OVERLAP = 10
# COFA count is dynamic — depends on whether Platform's COFA pipeline has run
# for this tenant/run. Not part of Farm seed ground truth.
...
def test_cofa_adjustments(client):
    """GET cofa-adjustments — 200, returns list of COFA conflicts."""
    resp = client.get("/api/dcl/reports/v2/cofa-adjustments", params={"period": "2025-Q1", **_TR})
    assert resp.status_code == 200
    data = resp.json()
    assert isinstance(data, list)
    # COFA conflicts are generated by Platform's engagement pipeline, not Farm.
    # Count varies based on whether COFA unification has been run for this tenant/run.
    for conflict in data:
        assert "conflict_id" in conflict, f"COFA conflict missing conflict_id: {conflict}"
