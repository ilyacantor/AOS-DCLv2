...
class TestSeedData:
    ...
    # 9. COFA conflicts (if present) have required properties
    def test_09_cofa_adjustments(self, conn):
        """COFA conflict triples, if present, have required properties.

        COFA conflicts are generated by Platform's engagement pipeline,
        not by Farm seed. They may or may not exist depending on whether
        COFA unification has been run for this tenant.
        """
        cur = conn.cursor()
        cur.execute(
