...
31. 2026-04-19 | snapshot-pollution-fix session | backend/api/main.py:619-620, backend/api/routes/reconciliation.py:137-138,516 | `app.state.loaded_sources` and `app.state.loaded_source_ids` are process-global variables set in `run_dcl()` and read in SOR and Farm reconciliation. Same class of cross-entity bleed as `app.state.aam_snapshot_name` (fixed this session) — whichever entity ran last overwrites the source list, causing SOR recon to show the wrong entity's sources. Fix: replace with per-entity source list from `current_triples` or `tenant_runs` enrichment columns. Severity: degraded. Blocking: SOR reconciliation accuracy under multi-entity DCL.
