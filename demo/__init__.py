"""
Grounded-agent before/after demo (ContextOS_Blueprint_v1 §13) — DCL's
successor to the Sankey semantics demo.
...
must never import demo.* — Panel A in particular is an operator-gated tool,
not an API, and must not become an importable data path. Enforced by
